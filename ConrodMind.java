//A Mind for ConrodWorld
import java.util.*;
import java.awt.Point;
import org.w2mind.net.*;


public class ConrodMind implements Mind {
	...
	*/
	public Action getaction(State state) {
		...
		if(Math.abs(distanceX) > Math.abs(distanceY)) {
			...
			if(distanceX < 0) {
				action = ConrodWorld.ACTION_DOWN;
			} else {
				action = ConrodWorld.ACTION_UP;
			}
		} else {
			...
			if(distanceY < 0) {
				//mouse is below cat
				action = ConrodWorld.ACTION_RIGHT;
			} else {
				//mouse is to the left or in line of cat
				action = ConrodWorld.ACTION_LEFT;
			}
		}
